...
There's also a **💾 Storage pane** (drive button in the header): the biggest files quietly sitting in Downloads, Desktop, Documents and Movies — size, location, how long you've ignored them — with a free-disk-space bar in the corner. From there, open the **Disk Map**: a full window with a clickable pie of what's eating your disk — drill into folders wedge by wedge, scan any folder you pick. For your files, there is deliberately **no delete button**: rows only *reveal in Finder*, because deleting your data is a decision bgviewer refuses to make for you.

The one exception is the **Dev Junk** view (second tab of the Disk Map): regenerable build artifacts — `node_modules`, Python venvs, Rust `target`, CocoaPods, `.next`/`.turbo`, Xcode DerivedData — plus **app caches** (Adobe media cache, Xcode device support, npm/pip/Homebrew caches, and any non-Apple `~/Library/Caches` folder over 100 MB), all of which rebuild on the next install or launch. Those it can delete, behind strict guards (a folder only qualifies with its manifest beside it — `package.json`, `Cargo.toml`, `pyvenv.cfg`…), a guard re-check at delete time, and a two-step confirm. Ten projects × 1 GB of duplicate dependencies adds up; this is where you get it back.

Beyond the buttons: **click a row** for details — the full command line, *Copy command*, *Reveal plist*, *View log*, and for dev servers a one-click *open localhost:port*. There's a **search box** (name, port, or command) and the list **auto-refreshes every few seconds while open**. The **⚙ Settings panel** holds **Alerts** (a notification the moment a new dev server starts listening, even while the dropdown is closed), **start at login**, and updates: when a newer release exists a green button appears in the header, and one click **updates the app in place** — checksum-verified against the release's `checksums.txt`, signature-checked, with automatic rollback. Flip **auto-update** on and new releases install themselves. Release checks hit GitHub at most every 6 hours and updates come only from this repo's Releases — no telemetry, ever. All of this is also explained in-app — the **ⓘ** button top-left.
